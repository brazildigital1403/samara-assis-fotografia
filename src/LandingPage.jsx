...
const LANDING_CSS = `
  ...
  .lp-page { font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif; min-height: 100vh; background: #fff; color: #000; }

  /* HEADER */
  .lp-header { background: rgba(255,255,255,0.95); backdrop-filter: blur(8px); border-bottom: 1px solid #f0f0f0; padding: 0.75rem 1rem; position: sticky; top: 0; z-index: 100; }
  .lp-header-inner { max-width: 1200px; margin: 0 auto; display: flex; align-items: center; justify-content: space-between; gap: 1rem; }
  .lp-header-brand { display: flex; align-items: center; gap: 0.75rem; min-width: 0; }
  ...
  .lp-header-nav { display: flex; gap: 0.25rem; align-items: center; }
  .lp-header-nav a { padding: 8px 12px; font-size: 13px; font-weight: 600; color: #555; text-decoration: none; border-radius: 6px; transition: all 0.2s; white-space: nowrap; }
  .lp-header-nav a:hover { background: #f5f5f5; color: #000; }
  .lp-header-nav .lp-cta { background: #000; color: white; padding: 8px 16px; }
  .lp-header-nav .lp-cta:hover { background: #333; color: white; }
  @media (max-width: 720px) {
    .lp-header-brand p { display: none; }
    .lp-header-nav a { padding: 8px 10px; font-size: 12px; }
    .lp-header-nav a.lp-nav-mob-hide { display: none; }
  }

  /* HERO */
  .lp-hero { padding: 5rem 1.5rem; text-align: center; background: linear-gradient(180deg, #fafafa 0%, #fff 100%); border-bottom: 1px solid #f0f0f0; }
  ...
  .lp-hero-cta { display: inline-flex; align-items: center; gap: 10px; padding: 16px 32px; background: #000; color: white; border: none; border-radius: 12px; font-size: 16px; font-weight: 700; cursor: pointer; transition: all 0.2s; text-decoration: none; }
  ...
  .lp-section-title { font-size: 32px; font-weight: 800; margin: 0 0 0.5rem; color: #000; letter-spacing: -0.5px; text-align: center; }
  .lp-section-subtitle { font-size: 14px; color: #999; margin: 0 0 3rem; text-align: center; text-transform: uppercase; letter-spacing: 1.5px; }
  ...
  /* BLOCO DE TEXTO */
  .lp-text-block { background: white; border: 1px solid #f0f0f0; border-radius: 16px; padding: 2rem; box-shadow: 0 2px 12px rgba(0,0,0,0.04); }
  .lp-text-block p { font-size: 15px; line-height: 1.8; color: #444; margin: 0 0 1rem; }
  .lp-text-block p:last-child { margin-bottom: 0; }
  .lp-text-block strong { color: #000; }
  .lp-text-block ul { padding-left: 0; list-style: none; margin: 0; }
  .lp-text-block li { font-size: 15px; line-height: 1.7; color: #444; padding: 0.6rem 0 0.6rem 1.75rem; position: relative; border-bottom: 1px solid #f5f5f5; }
  ...
  .lp-text-block .lp-warning { background: #fff3cd; border: 1px solid #ffe69c; padding: 1rem 1.25rem; border-radius: 8px; margin: 1rem 0; font-size: 14px; color: #5d4d00; }
  @media (max-width: 600px) {
  ...
  .lp-accordion-header { width: 100%; padding: 1rem 1.25rem; background: white; border: none; cursor: pointer; display: flex; align-items: center; justify-content: space-between; font-size: 16px; font-weight: 700; color: #000; text-align: left; transition: background 0.15s; }
  .lp-accordion-header:hover { background: #fafafa; }
  .lp-accordion-header.is-open { background: #000; color: white; }
  .lp-accordion-content { padding: 1.25rem 1.5rem 1.5rem; font-size: 14px; line-height: 1.7; color: #555; border-top: 1px solid #f0f0f0; background: #fafafa; }
  .lp-accordion-arrow { transition: transform 0.2s; }
  ...
  .lp-contato-info { text-align: center; margin-top: 2rem; padding-top: 2rem; border-top: 1px solid #f0f0f0; font-size: 13px; color: #888; line-height: 1.7; }
  .lp-contato-info strong { color: #000; }
...
function AccordionItem({ titulo, texto }) {
  const [open, setOpen] = React.useState(false);
  return (
    <div className="lp-accordion-item">
      <button onClick={() => setOpen(!open)} className={`lp-accordion-header ${open ? 'is-open' : ''}`}>
        <span>{titulo}</span>
        <ChevronDown size={18} className={`lp-accordion-arrow ${open ? 'is-open' : ''}`} />
      </button>
      {open && <div className="lp-accordion-content">{texto}</div>}
    </div>
  );
}
  ...
  return (
    <>
      ...
      <div className="lp-page">
        ...
        <header className="lp-header">
          <div className="lp-header-inner">
            ...
            <nav className="lp-header-nav">
              <a href="#informacoes" className="lp-nav-mob-hide">Informações</a>
              <a href="#orientacoes" className="lp-nav-mob-hide">Orientações</a>
              <a href="#contato" className="lp-nav-mob-hide">Contato</a>
              <a href="#" onClick={(e) => { e.preventDefault(); onIrParaCatalogo(); }} className="lp-cta">Catálogo</a>
            </nav>
          </div>
        </header>
        ...
        {/* INFORMAÇÕES */}
        <section id="informacoes" className="lp-section lp-section-divider" style={{ maxWidth: 'none', background: '#fafafa' }}>
        ...
        <section id="orientacoes" className="lp-section">
          ...
          <div className="lp-text-block">
            ...
            <p style={{ textAlign: 'center', fontStyle: 'italic', marginTop: '1.5rem' }}>Com certeza vocês irão se divertir e tudo vai acontecer naturalmente! ✨</p>
          </div>
        </section>

        {/* BEBÊ MÊS A MÊS */}
        <section id="bebe-mes-a-mes" className="lp-section lp-section-divider" style={{ maxWidth: 'none', background: '#fafafa' }}>
          <div style={{ maxWidth: 900, margin: '0 auto' }}>
            ...
            <p style={{ textAlign: 'center', color: '#666', fontSize: 14, lineHeight: 1.7, margin: '0 0 2rem', maxWidth: 700, marginLeft: 'auto', marginRight: 'auto' }}>
            ...
            <div className="lp-accordion">
              {BEBE_MESES.map(b => <AccordionItem key={b.mes} titulo={b.titulo} texto={b.texto} />)}
            </div>
            <p style={{ textAlign: 'center', color: '#999', fontSize: 11, marginTop: '2rem', fontStyle: 'italic', lineHeight: 1.6 }}>
        ...
        <section id="gemeos" className="lp-section">
          ...
          <div className="lp-text-block">
            <p style={{ textAlign: 'center', fontWeight: 600 }}>No momento atendemos ensaios de gêmeos para <strong>Menino e Menina</strong>:</p>
            <div className="lp-temas-grid">
              {TEMAS_GEMEOS.map(t => <div key={t} className="lp-tema-tag">{t}</div>)}
            </div>
            <p style={{ textAlign: 'center', marginTop: '2rem', fontSize: 13, color: '#888' }}>
        ...
        {/* CONTATO */}
        <section id="contato" className="lp-section lp-section-divider" style={{ maxWidth: 'none', background: '#fafafa' }}>
          <div style={{ maxWidth: 900, margin: '0 auto' }}>
        ...
        <section className="lp-section" style={{ textAlign: 'center', paddingTop: '2rem', paddingBottom: '5rem' }}>
          <h2 className="lp-section-title" style={{ marginBottom: '1.5rem' }}>Pronto para criar memórias?</h2>
          <button onClick={onIrParaCatalogo} className="lp-hero-cta">
            <Camera size={20} />
            Ver Catálogo de Cenários
            <ArrowRight size={20} />
